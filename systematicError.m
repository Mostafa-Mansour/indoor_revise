...
%%%%%%%%samples  %%%%%%%%%%%%%%%
samplePoints=repmat([a;a;0],1,samples)+repmat([(width_x-2*a);(length_y-2*a);1],1,samples).*rand(3,samples);
%%%%%%%%real values%%%%%%%%%%
x_real=1;
y_real=4;
c_real=0.8;
...
%                cov__=reshape(reshape(cov__,[1,40000]).*repmat(posterior(ii+1,:),1,4),[2,2,10000]);
               ...
               P(:,:,ii)=sum(cov__,3)-[X(1,ii);X(2,ii);X(3,ii)]*[X(1,ii),X(2,ii),X(2,ii)];
