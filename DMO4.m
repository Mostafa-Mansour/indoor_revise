%Рекуррентная (субоптимальная) схема реализации NOE
...
X=zeros(RazmX,1); %математическое ожидание
 
randn('state',0); rand('state',0); %установка датчика случайных чисел в начальное состояние

b0=50; Px=diag([b0^2 b0^2]);

P_NOE=zeros(RazmX,RazmX,m);P_NOE1=P_NOE;G_NOE1=P_NOE1;
x1=zeros(L,1); x2=zeros(L,1);

xj1=zeros(1,j1); xj2=zeros(1,j1);
Sij1=zeros(m,j1); Sij2=zeros(m,j1); Sij3=zeros(m,j1); Sij4=zeros(m,j1); Sij5=zeros(m,j1); Sij6=zeros(m,j1); 
Sij=zeros(4,j1);

% Запись рабочей области в файл
SaveWorkSpace=0;
...
%истинное значение координат объекта
x1=mX+b0*randn(1,L); 
x2=mX+b0*randn(1,L);
...
%vfnhbws lbyfvbrb b gjhj;lf.ob[ шумов
...
P3=zeros(2,2);G3=P3;
 ...
 %Формирование выборки метода Монте-Карло и соответствующей функции измерений

        xj1=mX+b0*randn(1,j1);  
        xj2=mX+b0*randn(1,j1);
...
l
   ...
   y1=sqrt((x1(l)-(Or1(1)+Ed11(l)))^2+(x2(l)-(Or1(2)+Ed12(l)))^2 + h1^2) + Ci1d(l) + v1(:,l); %формирование измерений
   
   y3=sqrt((x1(l)-(Or2(1)+Ed21(l)))^2+(x2(l)-(Or2(2)+Ed22(l)))^2 + h1^2) + Ci2d(l) + v3(:,l); %формирование измерений
   
   y5=sqrt((x1(l)-(Or3(1)+Ed31(l)))^2+(x2(l)-(Or3(2)+Ed32(l)))^2 + h1^2) + Ci3d(l) + v5(:,l); %формирование измерений
   
   y7=sqrt((x1(l)-(Or4(1)+Ed41(l)))^2+(x2(l)-(Or4(2)+Ed42(l)))^2 + h1^2) + Ci4d(l) + v7(:,l); %формирование измерений
 ...
 % y=[y3;y4;y5;y6];
  ...
  for k=1:m 
        ...
        for h=1:j1
         ...
         OzX = (1/j1)*Exp2/(Exp1); %оценка
          
         P = (1/j1)*Exp3/Exp1 -OzX*OzX'; % расчетная ковариация
                 
         GozX = (x(:,l)- OzX)*(x(:,l)- OzX)'; %действительная точность     
 ...
 P_NOE1(:,:,k)=P2(1:2,1:2)+P_NOE1(:,:,k); %суммирование для последующего осреднения
 
 P3=P+P3;
 
 G_NOE1(:,:,k)=GozX2(1:2,1:2)+G_NOE1(:,:,k);%суммирование для последующего осреднения
 ...
 P_NOE1=P_NOE1/L; %расчетная
 
 G_NOE1=G_NOE1/L;%действительная
